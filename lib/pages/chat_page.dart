import 'package:chat/pages/image_page.dart';
...
class _ChatPageState extends State<ChatPage> {
  ...
  void sendMessage() async {
    ...
    if (_messageController.text.isNotEmpty) {
      ...
      await _chatService.sendMessage(
          ...
          false); // Отправка сообщения через chatService
      _messageController
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        // Шапка приложения
        title: Text(
            widget.receiveuserEmail), // Отображение email получателя в шапке
    ...
    return StreamBuilder(
      ...
      stream: _chatService.getMessages(
          ...
          _firebaseAuth.currentUser!
              .uid), // Инициализация потока сообщений от сервиса чата
      builder: (context, snapshot) {
        ...
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          return const Text(
              'loadin...'); // Отображение текста, указывающего на процесс загрузки
        }
  ...
  Widget _buildMessageItem(DocumentSnapshot document) {
    ...
    return GestureDetector(
      onTap: () {
        if (data["isImage"].toString() == "true") {
          Navigator.push(
            context,
            MaterialPageRoute(
                builder: (context) => ImagePage(imgUrl: data["message"])),
          );
        } else {
  ...
  Widget _buildMessageInput() {
    return Row(children: [
      ...
      Expanded(
        ...
        child: Padding(
          padding: const EdgeInsets.only(
              left: 10, bottom: 10, right: 0), // Отступы вокруг поля
          child: MyTextField(
            // Мой собственный класс текстового поля, который, вероятно, расширяет функциональность базового TextField
            controller:
                _messageController, // Контроллер для управления текстом в поле ввода
      ...
      Padding(
        padding:
            const EdgeInsets.only(bottom: 13, left: 5), // Отступы для кнопки
        child: IconButton(
      ...
      Expanded(
        ...
        child: Padding(
          padding:
              const EdgeInsets.only(bottom: 20, left: 5), // Отступы для кнопки
          child: IconButton(
